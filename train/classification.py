...
class Classification(LogDB):
    ...
    def training(self, tuning=False):
        ...
        return self.model.state_dict(), train_metrics['loss']
    def evaluate(self, model=None):
        if model is None:
            model = self.model
        valid_metrics = self.trainer.test(self.test_loader, model, self.res)
        for key, value in valid_metrics.items():
            print(f'{key}: {value:6.4f}', end=', ')
        return valid_metrics

    def update_model_state_dict(self, new_state_dict):
        """
        更新模型的状态字典
        :param new_state_dict: 新的状态字典
        """
